...
class NamedScopeTest < ActiveRecord::TestCase
  ...
  def test_chaining_should_use_conjunction_of_all_where_conditions_when_searching
    # Normal hash conditions
    assert_equal [posts(:thinking)], Post.with_ids([posts(:welcome).id, posts(:thinking).id]).with_ids([posts(:thinking).id, posts(:authorless).id])
